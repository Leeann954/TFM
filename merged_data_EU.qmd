...
summary(iv_model)
msummary(
  list('First Stage' = iv_model$iv_first_stage[[1]],
       'Second Stage' = iv_model),
  coef_map = c(patent_lag1 = 'Lagged Patents',
...
results_table_long %>% 
  kable("html",
        ...
        align     = align_vec,
        col.names = NULL) %>% 
  kable_classic_2(full_width = FALSE, html_font = "Calibri", font_size = 14) %>% 
  column_spec(1, width = "5cm", extra_css = "text-align:left;") %>% 
  column_spec(2:ncol(results_table_long), extra_css = "text-align:center;") %>% 
  add_header_above(
    c(" " = 1,
      "OLS"  = 1,
      "Within" = 1,
...
mean_array <- results_lp$irf_lin_mean
low_array  <- results_lp$irf_lin_low
up_array   <- results_lp$irf_lin_up

horizons <- 0:(dim(mean_array)[2] - 1)

est_rd_to_growth <- mean_array[2, , 1]
low_rd_to_growth <- low_array[2, , 1]
up_rd_to_growth  <- up_array[2, , 1]

df_irf <- data.frame(
  h        = horizons,
  estimate = est_rd_to_growth,
  lower    = low_rd_to_growth,
  upper    = up_rd_to_growth
)
df_irf1 <- df_irf %>% filter(h <= 2)

ggplot(df_irf1, aes(x = h, y = estimate)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = "#66c2a5", alpha = 0.3) +
  geom_line(color = "#238b45", size = 1) +
  geom_point(color = "#238b45", size = 2) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray40") +
  scale_x_continuous(breaks = df_irf$h) +
  ylim(-0.002, 0.004) +  
  labs(
    title = "IRF of RD Shock on Growth ",
    x     = "Horizon (h)",
    y     = "Impulse Response"
  ) +
...
gmm_pvals_fmt %>%
  ...
  column_spec(2:4, width = "2cm", extra_css = "text-align:center;")
