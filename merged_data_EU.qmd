...
t.test(patent_count ~ industry_group, data = data_2019, alternative = "greater")

ggplot(data_2019, aes(x = industry_group, y = patent_count)) +
  geom_boxplot() +
  labs(title = "Patens：Top20% industry vs others", y = "Patent Count", x = "") +
  theme_minimal()

wilcox.test(sales_growth_1y_percent ~ industry_group, data = data_2019, alternative = "greater")

ggplot(data_2019, aes(x = industry_group, y = sales_growth_1y_percent)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha = 0.3, color = "steelblue") +
  stat_summary(fun = mean, geom = "point", shape = 21, size = 3, fill = "red") +
  coord_cartesian(ylim = c(-100, 300)) +  
  labs(title = "Sales Growth : Top20% vs Others",
       y = "1-Year Sales Growth (%)", x = "") +
  theme_minimal()

```

Based on 2019 data, firms in the top 20% R&D-intensive industries demonstrate significantly higher innovation output and stronger growth performance compared to others. A Welch two-sample t-test shows that their average patent count is significantly greater (p = 0.0161), and a Wilcoxon rank-sum test confirms that their sales growth is also significantly higher (p = 0.0009). These results support the hypothesis that operating in more R&D-intensive industries is associated with both increased innovation and better firm-level growth outcomes.
...
t.test(patent_count ~ country_group, data = data_2019, alternative = "greater")

ggplot(data_2019, aes(x = country_group, y = patent_count)) +
  geom_boxplot() +
  labs(title = "Patents: Top20% countries vs Others", y = "Patent Count", x = "") +
  theme_minimal()
wilcox.test(sales_growth_1y_percent ~ country_group, data = data_2019, alternative = "greater")

ggplot(data_2019, aes(x = country_group, y = sales_growth_1y_percent)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha = 0.3, color = "steelblue") +
  stat_summary(fun = mean, geom = "point", shape = 21, size = 3, fill = "red") +
  coord_cartesian(ylim = c(-100, 300)) +
  labs(title = "Sales Growth: Top20% countries vs Others",
       y = "1-Year Sales Growth (%)", x = "") +
  theme_minimal()

```

Firms in the top 20% R&D-intensive countries did not show significantly higher patent counts or sales growth compared to others. The t-test (p = 0.7063) and Wilcoxon test (p = 0.1253) both indicate no statistical difference, suggesting that country-level R&D intensity has limited impact on firm-level outcomes.

The empirical findings reveal a notable distinction between country-level and industry-level effects of R&D intensity. While firms in the top 20% R&D-intensive industries exhibit significantly higher innovation output and sales growth, no such relationship is observed at the country level. Specifically, neither patent counts nor sales growth differ significantly between firms from high-R&D countries and others. This suggests that the transmission of R&D intensity to firm-level performance is more strongly conditioned by industry-specific technological dynamics than by national R&D environments. 
...
summary(ols_model)
```
The OLS regression shows that lagged R&D intensity  has a positive and statistically significant effect on firm growth (p < 0.001), suggesting that past R&D investment contributes to future sales growth. In contrast, lagged firm growth  is negatively associated with current growth, which may indicate a mean-reversion effect. Firm size  has a weakly negative effect and is not statistically significant at the 5% level. The model controls for industry, country, and year fixed effects, and explains about 42% of the variation in growth .
...
panel_data <- panel_data %>%
  ...
  mutate(
    patent_lag1 = dplyr::lag(patent_count, order_by = year)
  ) %>%
  ungroup()
...
To address endogeneity concerns, I employed a two-stage least squares (2SLS) approach, using lagged patent counts as an instrumental variable for R&D intensity. The results show that the predicted R&D intensity has no significant effect on firm growth (p = 0.486), and the first-stage F-statistic is low (F = 0.10), indicating a weak instrument.
...
Using a two-step System GMM model on a panel of 417 firms from 2010 to 2019, I find that lagged R&D intensity has a significant and positive effect on firm sales growth. This indicates that prior R&D investment contributes meaningfully to subsequent firm performance. The model also reveals a slight mean-reverting tendency in sales growth, along with mixed effects of firm size—current size negatively affects growth, while lagged size has a positive influence. The AR(2) test suggests no second-order autocorrelation, supporting the validity of the model specification. 
...
The result examines the dynamic relationship between R&D intensity and sales growth using local projection models. Results show R&D spending significantly boosts sales growth over 5 periods (all p-values <0.01), with effects peaking in period 5 (p=3.55e-08), suggesting cumulative benefits. Conversely, sales growth shows no significant impact on R&D decisions (all p-values >0.9). This confirms R&D as a key driver of future sales growth, without reverse causality.
...
# instrum_var <- data.frame(patent_count = as.numeric(merged_data1$patent_count))
# instrum_var <- data.frame(patent = as.numeric(merged_data1$patent_count + 0))
...
Even when I pass in the tool (patent_count) as a properly formatted data frame (no NA, correct length, confirmed with str() and class()), the function still fails. I also tested a minimal example and reinstalled the latest GitHub version of lpirfs, but the problem persists.  /(TOT)/~~ ORZ
